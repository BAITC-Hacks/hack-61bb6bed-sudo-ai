...
В `.env.example` сейчас есть пример модели и включён технический демо-режим.
Замените их значениями выше. Для работы AI впишите свой ключ и доступную модель
...
docker compose exec -T backend python < backend/scripts/seed_notification_demo.py
```

Сначала запускается база и обновляется её схема, затем сервер и интерфейс.
Скрипты добавляют тестовые аккаунты, задачи, команды и один пример отклика.
Повторный запуск не создаёт дубликаты и не делает прочитанный отклик новым.
...
1. Запустите проект и скрипты из предыдущего раздела.
